# References

## Technical References

- [SwiftWebUI](https://github.com/SwiftWebUI/SwiftWebUI)
- [the-super-tiny-compiler](https://github.com/jamiebuilds/the-super-tiny-compiler)
- [Syntax Highlight Guide for VSCode](https://code.visualstudio.com/api/language-extensions/syntax-highlight-guide)
- [gogocode](https://github.com/thx/gogocode)
- [xstate state machine](https://github.com/davidkpiano/xstate)
- [hookstate](https://github.com/avkonst/hookstate)
- [mini-vue](https://github.com/cuixiaorui/mini-vue)
- [ficusjs, create webcomponents without pains](https://github.com/ficusjs/ficusjs)
- [TypeScript to WebAssembly](https://github.com/AssemblyScript/assemblyscript)
- [在JavaScript中使用Swift的尾闭包语法](https://github.com/dreamapplehappy/blog/blob/master/2021/04/05/README.md)
